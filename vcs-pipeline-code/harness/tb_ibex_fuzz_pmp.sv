...
module tb_ibex_fuzz;
  ...
  ibex_top #(
    ...
    .instr_rdata_i(imem_rdata), .instr_gnt_i(1'b1), .instr_rvalid_i(1'b1), .instr_err_i(1'b0),
    .data_req_o(dmem_req), .data_addr_o(dmem_addr),
    .data_wdata_o(dmem_wdata), .data_we_o(dmem_we), .data_be_o(dmem_be),
    .data_rdata_i(dmem_rdata), .data_gnt_i(1'b1), .data_rvalid_i(1'b1), .data_err_i(1'b0),
    .irq_software_i(1'b0), .irq_timer_i(1'b0), .irq_external_i(1'b0),
    .debug_req_i(1'b0), .fetch_enable_i(IbexMuBiOn),
  ...
  initial begin
    ...
    // Pre-fill memory with NOPs (addi x0,x0,0) so unloaded regions are benign
    for (int i = 0; i < 65536; i++) mem[i] = 32'h00000013;
